...
def virsh_command():
    ...
    """
    touch_lvm_src = terminal_os.popen("sudo virsh domblklist " + kvm_vm_name + " --details").read().split()

    if terminal_os.popen("sudo virsh domstate " + kvm_vm_name).read().split() == ["работает"]:
        terminal_os.popen("sudo virsh save " + kvm_vm_name + " " + touch_folder_src + ".vmstate" + " --running")
    elif terminal_os.popen("sudo virsh domstate " + kvm_vm_name).read().split() == ["выключен"]:
        terminal_os.popen("sudo virsh dumpxml " + kvm_vm_name + " > " + touch_folder_src + ".xml") 
        terminal_os.popen("sudo virsh domblkinfo " + kvm_vm_name + " " + touch_lvm_src[8] + " > " + touch_folder_src + 
...
def virsh_restore():
    ...
    if terminal_os.popen("sudo virsh domstate " + kvm_vm_name).read().split() == ["выключен"]:
        terminal_os.popen("sudo virsh restore " + touch_folder_src + ".vmstate")
...
def archive_creation(compression = 3):
    ...
    """
    terminal_os.popen("sudo dd if=" + touch_lvm_src + "_snap" + " | gzip -ck -" + str(compression) + " > " + touch_folder_src + ".gz").read()
...
def lvm_command(command, ratio = 2):
    ...
    """
    if command == 1:
        terminal_os.popen("sudo lvcreate -s -n " + touch_lvm_src + "_snap -L" + str(ratio*256) + "M " + touch_lvm_src)
        logs_creation("LVM Create " + touch_lvm_src + "_snap Size: " + str(ratio*256) + "M Target:" + touch_lvm_src)
    elif command == 2:
        terminal_os.popen("sudo lvremove -f " + touch_lvm_src + "_snap")
        logs_creation("LVM Remove " + touch_lvm_src + "_snap")
    elif command == 3:
        return terminal_os.popen("sudo lvdisplay " + touch_lvm_src + "_snap").read()
